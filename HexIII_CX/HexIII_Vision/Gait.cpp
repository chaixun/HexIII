...
int CGait::RunGait(EGAIT& p_gait,Aris::RT_CONTROL::CMachineData& p_data)
{
    ...
    case GAIT_BEGIN_DISCOVER:
        ...
        if(p_gait!=m_currentGait)
        {
            rt_printf("GAIT_BEGIN_DISCOVER begin\n");
            m_gaitState=EGaitState::GAIT_RUN;
            m_currentGait=p_gait;
            m_gaitStartTime=p_data.time;
            for(int i=0;i<MOTOR_NUM;i++)
            {
                m_commandDataMapped[i].Position=GaitBeginDiscover[0][i];
            }
        ...
        else
        {
            ...
            for(int i=0;i<MOTOR_NUM;i++)
            {
                m_commandDataMapped[i].Position=GaitBeginDiscover[m_gaitCurrentIndex][i];
            }

            if(m_gaitCurrentIndex==GAIT_DISCOVER_LEN-1)
            {
                rt_printf("GAIT_BEGIN_DISCOVER will transfer to GAIT_STANDSTILL...");
                p_gait=GAIT_STANDSTILL;
                for(int i=0;i<MOTOR_NUM;i++)
                {
                    m_standStillData[i].Position=m_feedbackDataMapped[i].Position;
    ...
    case GAIT_END_DISCOVER:
        ...
        if(p_gait!=m_currentGait)
        {
            rt_printf("GAIT_END_DISCOVER begin\n");
            m_gaitState=EGaitState::GAIT_RUN;
            m_currentGait=p_gait;
            m_gaitStartTime=p_data.time;
            for(int i=0;i<MOTOR_NUM;i++)
            {
                m_commandDataMapped[i].Position=GaitEndDiscover[0][i];
            }
        ...
        else
        {
            ...
            for(int i=0;i<MOTOR_NUM;i++)
            {
                m_commandDataMapped[i].Position=GaitEndDiscover[m_gaitCurrentIndex][i];
            }

            if(m_gaitCurrentIndex==GAIT_DISCOVER_LEN-1)
            {
                rt_printf("GAIT_END_DISCOVER will transfer to GAIT_STANDSTILL...");
                p_gait=GAIT_STANDSTILL;
                for(int i=0;i<MOTOR_NUM;i++)
                {
                    m_standStillData[i].Position=m_feedbackDataMapped[i].Position;
    ...
    case GAIT_WALK_ADAPTIVE:
        ...
        if(p_gait!=m_currentGait)
        {
            rt_printf("GAIT_WALK_ADAPTIVE begin\n");
            m_gaitState=EGaitState::GAIT_RUN;
            m_currentGait=p_gait;
            m_gaitStartTime=p_data.time;
            for(int i=0;i<MOTOR_NUM;i++)
            {
                m_commandDataMapped[i].Position=GaitAdaptiveWalk[0][i];
            }
        }
        else
        {
            ...
            for(int i=0;i<MOTOR_NUM;i++)
            {
                m_commandDataMapped[i].Position=GaitAdaptiveWalk[m_gaitCurrentIndex][i];
            }

            if(m_gaitCurrentIndex==GAIT_ADAPTIVEWALK_LEN-1)
            {
                rt_printf("GAIT_WALK_ADAPTIVE will transfer to GAIT_STANDSTILL...");
                p_gait=GAIT_STANDSTILL;
                for(int i=0;i<MOTOR_NUM;i++)
                {
                    m_standStillData[i].Position=m_feedbackDataMapped[i].Position;
